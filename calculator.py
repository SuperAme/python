...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        self.buttonPlu = Button(self, bg = "#98DBC6", bd = 12,
        text = "+", padx = 36, pady = 25, font = ("Helvetica", 20, "bold"),
        command = lambda : self.buttonClick("+"))
        self.buttonPlu.grid(row = 2, column = 3, sticky = W)

        self.buttonSub = Button(self, bg = "#98DBC6", bd = 12,
        text = "-", padx = 39, pady = 25, font = ("Helvetica", 20, "bold"),
        command = lambda : self.buttonClick("-"))
        self.buttonSub.grid(row = 3, column = 3, sticky = W)

        self.buttonMul = Button(self, bg = "#98DBC6", bd = 12,
        text = "*", padx = 38, pady = 25, font = ("Helvetica", 20, "bold"),
        command = lambda : self.buttonClick("*"))
        self.buttonMul.grid(row = 4, column = 3, sticky = W)

        self.buttonDiv = Button(self, bg = "#98DBC6", bd = 12,
        text = "/", padx = 39, pady = 25, font = ("Helvetica", 20, "bold"),
        command = lambda : self.buttonClick("/"))
        self.buttonDiv.grid(row = 5, column = 1, sticky = W)

        self.buttonRes = Button(self, bg = "#98DBC6", bd = 12,
        text = "=", padx = 100, pady = 25,
        command = self.CalculateTask, font = ("Helvetica", 20, "bold"))
        self.buttonRes.grid(row = 5, column = 2, sticky = W, columnspan = 2)

        self.buttonCle = Button(self, bg = "#E6D72A", bd = 12,
        text = "AC",font = ("Helvetica", 20, "bold"), width = 28, padx = 7, command = self.ClearDisplay) 
        self.buttonCle.grid(row = 1, columnspan = 4, sticky = W)
    ...
    def CalculateTask(self):
        self.date = self.user_input.get()
        try:
            self.answer = eval(self.date)
